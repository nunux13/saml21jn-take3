...
static ssize_t _riot_board2_handler(coap_pkt_t *pkt, uint8_t *buf, size_t len)
{
    return coap_reply_simple(pkt, COAP_CODE_205, buf, len,
            COAP_FORMAT_TEXT, (uint8_t*)RIOT_BOARD, strlen(RIOT_BOARD));
}

static ssize_t _riot_light_handler(coap_pkt_t *pkt, uint8_t *buf, size_t len)
{
    ssize_t p = 0;
    char rsp[16];
    ...
    puts("RIOT light handler");
    switch(method_flag) {
    case COAP_GET:
        puts("RIOT light get");
        //p= strlen(RIOT_BOARD);
        //memcpy(rsp, (char*)RIOT_BOARD, p);
        p += fmt_u32_dec(rsp, gpio_read(LED0_PIN));
        code = COAP_CODE_205;
        break;
    case COAP_PUT:{
        puts("RIOT light put");
...
const coap_resource_t coap_resources[] = {
    ...
    { "/riot/board2", COAP_GET, _riot_board2_handler },
    { "/riot/light", COAP_GET | COAP_PUT | COAP_POST, _riot_light_handler },
